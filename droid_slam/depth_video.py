...
class DepthVideo:
    def __init__(self, image_size=[480, 640], buffer=1024, stereo=False, device="cuda:0"):
        ...
        self.variance_map = torch.zeros(buffer, ht//8, wd//8, 1, dtype=torch.half, device=device).share_memory_()
    ...
    def to(self, device="cuda"):
        ...
        self.variance_map = self.variance_map.to(device=device)
        return self
    ...
    def __item_setter(self, index, item):
        ...
        if len(item) > 9:
            self.variance_map[index] = item[9]
