...
class DroidFrontend:
    def __init__(self, net, video, args):
        ...
        self.graph = FactorGraph(
            video, net.update, net.defDliation, net.variance_predictor, max_factors=48, upsample=args.upsample
        )
    ...
    def _update(self):
        ...
        for itr in range(self.iters1):
            if itr == self.iters1 - 1:
                self.graph.update(None, None, update_iters=itr, isDef=isDef, if_vis=True, use_inactive=True)
            else:
                self.graph.update(None, None, update_iters=itr, isDef=isDef, if_vis=True, use_inactive=True)
            isDef=True
        ...
        if d.item() < 2 * self.keyframe_thresh:
            ...
            for itr in range(self.iters2):
                if itr == self.iters1 - 1:
                    self.graph.update(None, None, isDef=isDef, use_inactive=True)
                else:
                    self.graph.update(None, None, isDef=isDef, use_inactive=True)
                isDef=True
    ...
    def _initialize(self):
        ...
        isDef=False
        for itr in range(8):
            self.graph.update(1, isDef=isDef, use_inactive=True)
            isDef=True
