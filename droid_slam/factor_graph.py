...
# from visual_sample import visualize_sampling_process
...
class FactorGraph:
    def __init__(self, video, update_op, defDliation, variance_predictor, device="cuda:0", corr_impl="volume", max_factors=-1, upsample=False):
        self.video = video
        ...
        self.var_estimator = variance_predictor
        self.defDliation = defDliation
        ...
        self.corr, self.net, self.inp, self.variance_map = None, None, None,None
    ...
    def add_factors(self, ii, jj, remove=False):
        ...
        if self.corr_impl == "volume":
            ...
            corr = CorrBlock(self.defDliation, fmap1, fmap2)
              # free memory
         
            # if self.corr is not None:
            #     offset = self.defDliation(self.video.nets[ii].to(self.device).unsqueeze(0))
            # self.corr = corr if self.corr is None else self.corr.cat(corr, offset)

            self.corr = corr if self.corr is None else self.corr.cat(corr)
            
            inp = self.video.inps[ii].to(self.device).unsqueeze(0) # output of context decoder with net
            variance_map = self.video.variance_map[ii].to(self.device)

            self.inp = inp if self.inp is None else torch.cat([self.inp, inp], 1)
            self.variance_map = variance_map if self.variance_map is None else torch.cat([self.variance_map, variance_map], 0)
    ...
    def rm_factors(self, mask, store=False):
        ...
        if self.variance_map is not None:
            self.variance_map = self.variance_map[~mask]
    ...
    @autocast(enabled=True)
    def update(self, t0=None, t1=None, itrs=2, update_iters=None, if_vis=False, isDef=False, use_inactive=False, EP=1e-7, motion_only=False):
        ...
        if isDef:
            cur_ofs = self.defDliation(self.net)
            corr, offset = self.corr(coords1, self.variance_map, cur_ofs)
        else:
            corr, offset = self.corr(coords1, self.variance_map)
            
        # for i in range(len(self.ii)):
        #     self.video.offset_record[f'{self.ii[i].item()}_{self.jj[i].item()}'] = offset[0][i]
        # for i in range(len(self.ii)):
        #     self.video.offset_record1[f'{self.ii[i].item()}_{self.jj[i].item()}'] = offset[1][i]
        
        # if if_vis and update_iters is not None:
        #     left_images = self.video.images[self.ii]
        #     right_images = self.video.images[self.jj]
        #     inp_left = left_images[6]
        #     inp_right = right_images[6]
        #     coords1_inp = coords1.squeeze(0)[6]
        #     offset_inp = offset[0][6]
        #     save_name = f'/home/honsen/honsen/VO_SLAM/DROID-SLAM/sample_vis_kitti/sampling_visualization_{self.ii[6].item()}_{self.jj[6].item()}_iter_{update_iters}.png'
        #     visualize_sampling_process(inp_left, inp_right, coords1_inp, offset_inp, [(12,32),(28,16)], save_path=save_name)
