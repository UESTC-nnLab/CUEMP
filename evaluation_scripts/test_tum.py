...
if __name__ == '__main__':
    ...
    parent = "/home/honsen/tartan/TUMRGBD/"
    dir_list = sorted(os.listdir("/home/honsen/tartan/TUMRGBD"))

    for i in range(len(dir_list)):
        i = 2
        datapath = parent + dir_list[i]
        droid = Droid(args)
        time.sleep(5)

        tstamps = []
        for (t, image, intrinsics) in tqdm(image_stream(datapath)):
            # if not args.disable_vis:
            #     show_image(image[0])
            droid.track(t, image, None, intrinsics=intrinsics)

        traj_est = droid.terminate(image_stream(datapath))
